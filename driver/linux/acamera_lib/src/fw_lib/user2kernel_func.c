...
static void update_result_ae( user2kernel_fsm_ptr_t p_fsm, uint8_t ae_idx )
{
    ...
    rc = acamera_fsm_mgr_get_param( p_fsm->cmn.p_fsm_mgr, FSM_PARAM_GET_AE_STATE,
                               NULL, 0, &state, sizeof( state ) );
    if (rc) {
        LOG( LOG_ERR, "unable to get AE_STATE param rc: %d", rc );
        return;
    }
    p_sbuf_ae->state = state;
...
static void update_result_awb( user2kernel_fsm_ptr_t p_fsm, uint8_t awb_idx )
{
    ...
    p_sbuf_awb->awb_green_even_gain = ACAMERA_FSM2CTX_PTR( p_fsm )->stab.global_awb_green_even_gain;
    p_sbuf_awb->awb_green_odd_gain = ACAMERA_FSM2CTX_PTR( p_fsm )->stab.global_awb_green_odd_gain;
    ...
    rc = acamera_fsm_mgr_get_param( p_fsm->cmn.p_fsm_mgr, FSM_PARAM_GET_AWB_STATE,
                                    NULL, 0, &state, sizeof( state ) );
    if (rc) {
        LOG( LOG_ERR, "unable to get AWB_STATE param rc: %d", rc );
        return;
    }
    p_sbuf_awb->state = state;
...
static void update_result_af( user2kernel_fsm_ptr_t p_fsm, uint8_t af_idx )
{
    ...
    rc = acamera_fsm_mgr_get_param( p_fsm->cmn.p_fsm_mgr, FSM_PARAM_GET_AF_STATE,
                               NULL, 0, &state, sizeof( state ) );
    if (rc) {
        LOG( LOG_ERR, "unable to get AF_STATE param rc: %d", rc );
        return;
    }
    p_sbuf_af->state = state;
