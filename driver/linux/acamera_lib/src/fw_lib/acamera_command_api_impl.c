...
uint8_t offline_proc_frames_num( acamera_fsm_mgr_t *instance, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
    *ret_value = 0;
    ...
    if ( direction == COMMAND_SET ) {
        ...
        rc = acamera_fsm_mgr_set_param( instance, FSM_PARAM_SET_M2M_FRAMES_NUM, &val, sizeof( val ) );
        if ( rc ) {
            *ret_value = ERR_BAD_ARGUMENT;
            LOG( LOG_ERR, "return fail on bad argument %d\n", val );
            return FAIL;
        ...
        rc = acamera_fsm_mgr_get_param( instance, FSM_PARAM_GET_M2M_FRAMES_NUM, NULL, 0, &val, sizeof( val ) );
        *ret_value = val;
