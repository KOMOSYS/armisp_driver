...
typedef struct _af_acamera_core_obj_ {
    ...
    /*
     * is_focused - Marks if AF algo is in focus
     * Active only when af_status is TRACKING or CONVERGED
     */
    uint8_t is_focused;
...
int32_t af_acamera_core_proc( void *af_ctx, af_stats_data_t *stats, af_input_data_t *input, af_output_data_t *output )
{
    ...
    switch ( p_af_core_obj->af_status ) {
    case AF_STATUS_INVALID:
        state = AF_STATE_INACTIVE;
        break;
    case AF_STATUS_FAST_SEARCH:
        state = AF_STATE_SCAN;
        break;
    case AF_STATUS_TRACKING:
        /* Fall-through */
    case AF_STATUS_CONVERGED:
        state = p_af_core_obj->is_focused ? AF_STATE_FOCUSED : AF_STATE_UNFOCUSED;
        break;
    default:
        state = AF_STATE_INACTIVE;
        LOG( LOG_ERR, "unknown af_status: %d", p_af_core_obj->af_status );
        return -1;
    };

    p_af_acamera_output->state = state;
