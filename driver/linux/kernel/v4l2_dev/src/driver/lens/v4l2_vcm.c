...
static void lens_update_parameters( void *ctx )
{
    ...
    if ( p_ctx != NULL ) {
        ...
        if ( sd != NULL && ctx_num < FIRMWARE_CONTEXT_NUMBER ) {
            ...
            rc = v4l2_subdev_call( sd, core, ioctl, SOC_LENS_INFO_MINFOCUS_DISTANCE, &settings );
            p_ctx->param.minfocus_distance = settings.args.general.val_out;

            rc = v4l2_subdev_call( sd, core, ioctl, SOC_LENS_INFO_HYPERFOCAL_DISTANCE, &settings );
            p_ctx->param.hyperfocal_distance = settings.args.general.val_out;

            rc = v4l2_subdev_call( sd, core, ioctl, SOC_LENS_INFO_FOCAL_LENGTH, &settings );
            p_ctx->param.focal_length = settings.args.general.val_out;

            rc = v4l2_subdev_call( sd, core, ioctl, SOC_LENS_INFO_APERTURE, &settings );
            p_ctx->param.aperture = settings.args.general.val_out;
