...
static void sensor_update_parameters( void *ctx )
{
    ...
    if ( p_ctx != NULL ) {
        ...
        if ( sd != NULL && ctx_num < FIRMWARE_CONTEXT_NUMBER ) {
            ...
            rc = v4l2_subdev_call( sd, core, ioctl, SOC_SENSOR_INFO_PHYSICAL_WIDTH, &settings );
            p_ctx->param.physical_width = settings.args.general.val_out;

            rc = v4l2_subdev_call( sd, core, ioctl, SOC_SENSOR_INFO_PHYSICAL_HEIGHT, &settings );
            p_ctx->param.physical_height = settings.args.general.val_out;
