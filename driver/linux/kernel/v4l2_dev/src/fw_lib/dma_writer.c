...
static int dma_writer_configure_frame_reader( dma_pipe *pipe, tframe_t *frame )
{
    ...
    uint32_t address = aframe->address + pipe->settings.read_offset_y*aframe->line_offset + pipe->settings.read_offset_x;
    LOG(LOG_DEBUG, "offset %d %d, address 0x%x", pipe->settings.read_offset_x, pipe->settings.read_offset_y, aframe->address );
    pipe->api.p_acamera_fpga_frame_reader_rbase_write( pipe->settings.isp_base,address );
