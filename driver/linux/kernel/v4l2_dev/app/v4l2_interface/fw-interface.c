...
static int isp_fw_do_set_af_mode_id( uint32_t ctx_id, int val )
{
    int rc = -1;
    ...
    switch (val) {
    case ISP_V4L2_AF_MODE_AUTO_SINGLE:
        af_mode_id_val = AF_AUTO_SINGLE;
        break;
    case ISP_V4L2_AF_MODE_AUTO_CONTINUOUS:
        af_mode_id_val = AF_AUTO_CONTINUOUS;
        break;
    case ISP_V4L2_AF_MODE_MANUAL:
        af_mode_id_val = AF_MANUAL;
        break;
    default:
    ...
    rc = isp_fw_do_set_cmd( ctx_id, TALGORITHMS, AF_MODE_ID, af_mode_id_val );
    if ( rc ) {
        LOG( LOG_ERR, "Failed to set AF_MODE_ID val: %d, rc: %d", af_mode_id_val, rc );
        return rc;
    }
#endif
...
static int isp_fw_do_set_af_manual_control( uint32_t ctx_id, int val )
{
    int rc = -1;

#if defined( TALGORITHMS ) && defined( AF_MANUAL_CONTROL_ID )
    rc = isp_fw_do_set_cmd( ctx_id, TALGORITHMS, AF_MANUAL_CONTROL_ID, val );
    if ( rc ) {
        LOG( LOG_ERR, "Failed to set AF_MANUAL_CONTROL_ID val: %d, rc: %d", val, rc );
        return rc;
    }
#endif
...
int fw_intf_set_af_mode_id( uint32_t ctx_id, int val )
{
    return isp_fw_do_set_af_mode_id( ctx_id, val );
}

int fw_intf_set_af_manual_control( uint32_t ctx_id, int val )
{
    return isp_fw_do_set_af_manual_control( ctx_id, val );
}
